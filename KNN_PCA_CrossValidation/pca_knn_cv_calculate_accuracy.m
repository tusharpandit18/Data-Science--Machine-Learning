function [NN,accuracy, avg_result] = pca_knn_cv_calculate_accuracy(trainingData, trainingLabels, partitionSize)
    fold = 1;
    while fold <= 5
        train_set = [];
        test_set = [];
        ...
        distance_matrix = pca_knn_cv_Calc_dist_mat(train_set, test_set);
...
%             fprintf("knn_output %d",knn_output);
            final_matrix(fold,k) = pca_knn_cv_result(test_labels, knn_output);
            j = j + 1;
        end
